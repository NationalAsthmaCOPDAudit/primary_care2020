...
/* create log file */
capture log close
log using build_logs/17_AddVarsAsthmaChild_6-18, text replace
...
use "`data_dir'/builds/asthma_child_6-18_cohort_cm2", clear
...
//generate variable for smokers who've received BCI OR drug (DIFFERENT FOR CHILDREN)
gen byte smokbcidrug = (smokcess_bci == 1 | smokcess_drug == 1)
replace smokbcidrug = . if smokerlast2yrs == 0
replace smokbcidrug = . if age < 10   //only for those 10yrs and above
...
//not needed
drop obese_date gp_asthmaexac_count gp_asthmaexacs smokcess_bci smokcess_drug wimd_rank
...
compress
save "`data_dir'/builds/asthma_child_6-18_final", replace
